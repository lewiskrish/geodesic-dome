...
@njit
def adj_insert(adj: np.ndarray, root: np.int64, neighbour: np.int64):
    root_list = adj[root]
    for i in range(6):
...
@njit
def find_neighbours(vertices: np.ndarray, adj: np.ndarray, index: np.int64, depth: np.int64 = 1): 
    size = 0
    ...
    while (curr_depth <= depth):
        ...
        while q_front < q_end:
            # pop here, add neighbours, num_neighbours += neighbour count
            v_index = queue[q_front]
            # print("Current index: ", v_index)
            for neighbour in adj[v_index]:
                # print("Neighbour index: ", neighbour)
...
def create_icosphere(freq=0) -> Union[np.ndarray, np.ndarray]:
    ...
    adj_list = create_adj_list(icosa_vertices_normalised, icosa_triangles)

    return icosa_vertices_normalised, icosa_triangles, adj_list
...
def print_neighbours_json_array(neighbours: list) -> None:
    true_neighbours = [item for item in neighbours if item >= 0]
    string = '{"vertices": ['
    for vertex in true_neighbours:
        string += f"{vertex},"
    string = string[:-1]
    string += ']}'
    print(string)
...
def main():
    ...
    factor = info.split(",")[0]
    vertex = info.split(",")[1]
    distance = info.split(",")[2]
    vertices, triangles, adj_list = create_icosphere(int(factor))
    neighbours = find_neighbours(vertices, adj_list, int(vertex), int(distance))
    print_neighbours_json_array(neighbours)
